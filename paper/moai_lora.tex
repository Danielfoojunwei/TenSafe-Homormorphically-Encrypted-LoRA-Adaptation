...
{\small \textbf{Note}: These are \emph{per-layer} latencies. Full model inference requires multiplying by the number of layers applying LoRA. For Llama with LoRA on all attention projections (Q, K, V, O) across 32 layers: $12.24 \times 4 \times 32 = 1566$ seconds $\approx$ 26 minutes per token.}
...
We presented MOAI-LoRA, a rotation-free algorithm for encrypted LoRA inference that eliminates the dominant cost in CKKS-based computation. By exploiting LoRA's ciphertext-plaintext structure and column-packed matrix multiplication, we achieve \textbf{zero rotations} for single-block configurations and \textbf{$O(\log b)$} rotations for multi-block settings. Our analysis across Llama-2, Llama-3, Mistral, and GPT-J architectures shows rotation reductions of \textbf{2000$\times$+}, projecting to \textbf{8$\times$} wall-clock speedups. While encrypted LLM inference remains impractical for real-time applications, MOAI-LoRA represents a significant algorithmic improvement that reduces the gap.
...
For complete token generation (all layers, all attention projections):

\begin{table}[h]
\centering
\caption{Projected full-model inference time}
\begin{tabular}{lccc}
\toprule
...
These times are per-token and demonstrate that while MOAI-LoRA provides significant speedup, encrypted LLM inference remains far from practical for interactive use.
